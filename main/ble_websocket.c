...
{
    esp_websocket_client_handle_t client = (esp_websocket_client_handle_t)pv_parameter;
                                    ...
                                    void *event_data)
{
    ...
    case WEBSOCKET_EVENT_CONNECTED:
        ESP_LOGI(TAG, "WebSocket connected over %s: %s", s_use_ssl ? "WSS" : "WS", CONFIG_BLE_WEBSOCKET_URI);
...
static void wifi_init_sta(void)
{
    ...
    strncpy((char *)wifi_config.sta.ssid,
            CONFIG_BLE_WEBSOCKET_WIFI_SSID,
            sizeof(wifi_config.sta.ssid) - 1);
    strncpy((char *)wifi_config.sta.password,
            CONFIG_BLE_WEBSOCKET_WIFI_PASSWORD,
    ...
    ESP_LOGI(TAG, "Connecting to Wi-Fi SSID: %s", CONFIG_BLE_WEBSOCKET_WIFI_SSID);
...
static void websocket_start(void)
{
    ...
    ESP_LOGI(TAG, "WebSocket client started, target=%s", CONFIG_BLE_WEBSOCKET_URI);
}
...
void app_main(void)
{
    ...
    s_use_ssl = uri_uses_wss(CONFIG_BLE_WEBSOCKET_URI);
    reset_command_buffer();
    reset_upload_metrics();
